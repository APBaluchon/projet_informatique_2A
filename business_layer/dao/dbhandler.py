...
class DBHandler(metaclass=Singleton):
    ...
    This class provides methods to perform various database operations such as checking if a user exists,
        ...
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT * FROM projet_info.utilisateur u WHERE u.pseudo = %s", (pseudo,))
                    res = cursor.fetchone()
                    return bool(res)
    ...
    def create_user(self, pseudo, password):
        ...
        This method adds a new user with the specified pseudo and password to the database.
        ...
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("INSERT INTO projet_info.utilisateur VALUES (%s, %s, 'user')", (pseudo, password))
                    connection.commit()
                    return True
        except Exception as e:
    ...
    def is_password_correct(self, pseudo, password):
        """
        Checks if the provided password matches the one stored in the database for a given user.
        ...
        -------
        bool
            True if the password matches the one in the database, False otherwise.
        """
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT mdp FROM projet_info.utilisateur WHERE pseudo = %s", (pseudo,))
                    res = cursor.fetchone()
                    if res:
    ...
    def get_user_role(self, pseudo):
        ...
        -------
        str
            The role of the user in the database. Returns None if the user is not found.
        """
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT role FROM projet_info.utilisateur WHERE pseudo = %s", (pseudo,))
                    res = cursor.fetchone()
    ...
    def display_database(self):
        ...
        try:
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("SELECT pseudo, role FROM projet_info.utilisateur")
    ...
    def update_database(self):
        ...
        This method first prompts for the user's pseudo, and if the pseudo exists in the database,
        it then asks which aspect of the user's information should be updated (e.g., password, role). 
        Based on the choice, it delegates to the appropriate method for updating the specific information.
        """
        try:
    ...
    def update_password(self, pseudo_compte):
        ...
        try:
            ...
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("UPDATE projet_info.utilisateur SET mdp = %s WHERE pseudo = %s", (new_password, pseudo_compte))
                    connection.commit()
        except Exception as e:
    ...
    def update_role(self, pseudo_compte):
        ...
        try:
            ...
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("UPDATE projet_info.utilisateur SET role = %s WHERE pseudo = %s", (new_role, pseudo_compte))
                    connection.commit()
        except Exception as e:
    ...
    def delete_account_from_database(self):
        ...
        try:
            ...
            with DBConnection().connection as connection:
                with connection.cursor() as cursor:
                    cursor.execute("DELETE FROM projet_info.utilisateur WHERE pseudo = %s", (pseudo_compte,))
                    connection.commit()
